...
public interface SkillOrAbility {
    ...
    private static String abilityScore(JsonNode abilityNode, boolean increase) {
        ...
        if (choose == null) {
            return transform.streamOfFieldNames(abilityNode)
                    .filter(n -> !n.equalsIgnoreCase("max"))
                    .map(n -> {
                        String value = toAbilityString(n, abilityNode.get(n), increase);
                        return value.replace("MAX", max);
                    })
                    .collect(Collectors.joining(" "));
        }

        if (AsiChoiceFields.weighted.existsIn(choose)) {
            return AsiChoiceFields.weighted.readWeightedChoice(choose);
        } else if (AsiChoiceFields.from.existsIn(choose)) {
            return AsiChoiceFields.from.readFromChoice(choose, increase);
        }
        return "";
    }
    ...
    private static String toAbilityString(String nameAbv, JsonNode value, boolean increase) {
        if (increase) {
            return "Increase your %s score by %s, to a maximum of MAX.".formatted(
                    SkillOrAbility.fromTextValue(nameAbv),
                    value.asText());
        }
